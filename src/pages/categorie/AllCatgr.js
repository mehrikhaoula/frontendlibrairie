...
function AllCatgr() {
  ...
  const getAllCatgr = async () => {
    try {
      const fetchData = await fetch(`${process.env.REACT_APP_API_URL}/categories`, {
        method: 'GET',
        credentials: 'include',
      });
  ...
  const onDeleteCategorie = async (categorieId) => {
    ...
    try {
      const response = await axios.delete(`${process.env.REACT_APP_API_URL}/categorie/`+categorieId, {
    ...
    <div className="min-h-screen flex items-center justify-center py-10">
      <div className="w-full max-w-3xl rounded-lg shadow-gray-500 shadow-lg overflow-hidden">
        ...
        <div className="md:block">
          <CatgrTable categorie={categorie} onDelete={handleDeleted} onEdit={handleEditCatgr} />
        </div>
      </div>

      {ajouterCategorie && (
        <AjouterCatgr fetchdata={getAllCatgr} onClose={() => setAjouterCatgr(false)} />
      )}
      {viewDelete && (
        <DeleteCatgr
      ...
      {isEditing && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
          <EditCatgr categorie={selectedCatgr} onSave={handleUpdateCatgr} onCancel={handleCancel} />
        </div>
      )}
    </div>
  );
